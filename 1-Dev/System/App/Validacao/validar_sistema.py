...
def validate(root: Path) -> dict:
    ...
    required_files={"COPYRIGHT.txt","LICENSE.txt","CJL.exe","CJL.root.json","CJL.branch.json"}
    ...
    if not is_dev:
        ...
        if int(anchor.get("format") or 0)!=2 or anchor.get("product")!="CJL System" or int(anchor.get("revision") or 0)!=int(repo_head.get("revision") or 0): raise RuntimeError("Anchor CJL do Repo invalido.")
        if str(anchor.get("transaction_file_sha256") or "").lower()!=str(repo_head.get("transaction_file_sha256") or "").lower() or str(anchor.get("snapshot_sha256") or "").lower()!=str(repo_head.get("snapshot_sha256") or "").lower(): raise RuntimeError("Anchor CJL do Repo diverge do HEAD atual.")
    else:
